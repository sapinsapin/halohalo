...
| halohalo + BantayWika | fil, hil, bcl (+ilo, ceb) | curated literary and web text. `halo-hil` has been re-filtered (9,860 → 500 documents of verified Hiligaynon); the copy *inside* `halohalo` still carries the old unfiltered rows, so re-gate before use — see `reference/datasets-benchmarks.md` | per card |
